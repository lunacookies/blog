...
[A comment from u/witty___name](https://www.reddit.com/r/rust/comments/inoipw/something_that_blew_my_mind_about_scoped_variables/g4dd0ul?utm_source=share&utm_medium=web2x&context=3) clarified that this approach is problematic, though. (I’ll be paraphrasing their explanation here.)
...
1. *Calculates the size of all of `BindingDef`’s fields.*
...
Aaaand it doesn’t compile. It seems that the fields `lhs` and `rhs` of `Expr::Operation` can only be `crate::expr::Number`s. Although this *does* adhere to our initial design decision to disallow nested expressions, it prevents us from doing things as basic as defining an `add` function! We need to fix this.
